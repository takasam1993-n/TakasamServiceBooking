...
def cancel_appointment_db(app_id):
    ...
    if is_demo:
        ...
        if date_str in st.session_state.mock_db["time_slots"]:
            if time_str in st.session_state.mock_db["time_slots"][date_str]:
                st.session_state.mock_db["time_slots"][date_str].pop(time_str, None)
    ...
    try:
        response = supabase_client.rpc("cancel_dental_appointment", {"p_appointment_id": int(app_id)}).execute()
...
with st.sidebar:
    st.image("https://upload.wikimedia.org/wikipedia/commons/e/e4/MOPH_Logo.png", width=80)
    st.markdown("### รพ.สต.ท่าเกษม")
    st.markdown("🦷 **ระบบลงนัดทันตกรรม**")
    st.divider()
    ...
    if st.session_state.line_user_id:
        ...
        if is_demo:
            st.info("💡 ทำการจำลองสิทธิ์ LINE เพื่อจำลองการทำงานด้านล่าง")
            ...
            if st.button("บันทึกสิทธิ์จำลอง"):
...
if app_mode == "ผู้รับบริการ (LINE LIFF)":
    ...
    if not st.session_state.staff_logged_in:
        ...
        with col_sec_mid:
            ...
            passcode = st.text_input("รหัสผ่านเจ้าหน้าที่ (รหัสผ่านคือ 1234):", type="password")
            if st.button("ยืนยันรหัสผ่าน 🔑", use_container_width=True):
                if passcode == "1234":
                    ...
                    st.error("รหัสผ่านผู้ใช้งานไม่ถูกต้อง กรุณากรอกอีกครั้ง")
    else:
